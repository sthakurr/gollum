    ...
    def _llm_summary(self) -> str:
        ...
        for rec in self.history:
            for inp, score in zip(rec.input_texts, rec.scores):
                history_lines.append(f"  - score={score:.4f}  input={inp[:80]}")
        ...
        prompt = (
            ...
            "experiment over a discrete sequence search space.\n\n"
            f"Optimization history ({len(self.history)} iterations, "
            f"{sum(len(r.scores) for r in self.history)} experiments):\n"
            f"{history_text}\n\n"
            f"Global best so far: score={best_score:.4f}, input={best_input[:80]}\n\n"
            "In 2-3 sentences describe: (1) patterns shared by high-scoring "
            "inputs, (2) what regions of the search space appear promising, "
            "and (3) what the next candidate should look like.\n"
            "<|end|>\n<|assistant|>\n"
        )
